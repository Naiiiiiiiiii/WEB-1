  ...
  constructor({
    ...
    if (Array.isArray(items) && items.length > 0) {
      this.items = items.map((it) => ({
        ...
        totalValue:
          it.totalValue || Number(it.quantity) * Number(it.importPrice),
      }));
    } else if (productId) {
    ...
    this.totalValue =
      totalValue ||
      this.items.reduce(
        (s, it) => s + (it.totalValue || it.quantity * it.importPrice),
        0
      );
...
export class ImportManager {
  ...
  saveSlips() {
    try {
      ...
      syncToStorage("importSlips", this.slips);
      return true;
    } catch (error) {
  ...
  addSlip(slipData) {
    const newId =
      this.slips.length > 0 ? Math.max(...this.slips.map((s) => s.id)) + 1 : 1;
      ...
      prepared.items = [
        {
          ...
          totalValue:
            slipData.totalValue ||
            (slipData.quantity || 0) * (slipData.importPrice || 0),
        },
      ];
    } else {
      prepared.items = [];
  ...
  updateSlip(id, updatedData) {
    ...
    slip.supplier =
      updatedData.supplier !== undefined ? updatedData.supplier : slip.supplier;
