...
export function handleProductFormSubmit(e) {
  ...
  if (currentEditingProductId) {
    const existingProduct = productManager.getProductById(
...
function handleAddProduct(productData) {
  ...
  if (newProduct) {
    showNotification(
      `✅ Thêm sản phẩm "${newProductData.name}" thành công!`,
      "success"
    );
    closeProductModal();
    renderProductList();
...
function handleUpdateProduct(productId, productData) {
  ...
  if (success) {
    showNotification(
      `✅ Cập nhật sản phẩm "${productData.name}" thành công!`,
      "success"
    );
    closeProductModal();
    renderProductList();
    ...
    const rowHTML = `
            ...
            <td class="product-name">
                <div class="product-name-wrapper">
                    <img src="${p.img}" alt="${
      p.name
    }" class="product-thumb" onerror="this.src='./img/placeholder.png'">
                    <strong>${p.name}</strong>
                </div>
            </td>
            <td class="category-name">${categoryName}</td>
            <td class="right price-cell">${p.price.toLocaleString(
              "vi-VN"
            )} ₫</td>
            <td class="center stock-cell ${isLowStock ? "low-stock" : ""}">
                ...
                ${
                  isLowStock
                    ? '<i class="fa-solid fa-triangle-exclamation" title="Tồn kho thấp"></i>'
                    : ""
                }
            </td>
            <td>
            ...
            <td class="action-buttons">
                <button class="btn btn-info btn-view-product" data-id="${
                  p.id
                }" title="Xem chi tiết">
                ...
                <button class="btn btn-ghost ${
                  p.isHidden ? "btn-show" : "btn-hide"
                } btn-toggle-product" data-id="${p.id}" title="${
      p.isHidden ? "Hiển thị" : "Ẩn"
    } sản phẩm">
                    <i class="fa-solid ${
                      p.isHidden ? "fa-eye" : "fa-eye-slash"
                    }"></i>
                </button>
    ...
    product.name
                ...
                <div class="product-detail-info">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value price">${product.price.toLocaleString(
                          "vi-VN"
                        )} ₫</span>
                    </div>
                    <div class="detail-row">
...
function handleToggleProduct(e) {
  ...
  const confirmMessage = `Bạn có chắc muốn ${action.toUpperCase()} sản phẩm "${
    product.name
  }"?`;
